...
(defn start [app owner]
  (reify
    om/IDisplayName (display-name [_] "Roster Start")
...
(defn roster [app owner]
  (reify
    om/IDisplayName (display-name [_] "Roster")
    om/IRender
    (render [_]
      (let [cast! (om/get-shared owner :cast!)
            roster-components (map #(get roster-components %) (get-in app state/rosters-path))
            title (:title (last roster-components))]
        (html
         [:div.roster
          [:div.roster-header
           (for [component roster-components]
             (html
              [:h4
               {:title title}
               (:title component)]))]
