  ...
  (reify
    ...
    (render [_]
      (let [pan (cursors/observe-pan-tool owner)
        ...
        (html
          [:div.mouse.holo {:style {:top (:y mouse) :left (:x mouse)}}
           [:div.mouse-cursor
            (when panning?
              [:svg.pan-handle
            ...
            ;   [:div.hint {:style {:transform "translate3d(-50%, 100%, 0)"}} "Right-click."])
           ...
           [:div.hint (if (get-in app [:radial :open?])
                         {:data-chat "Open chat." :class "east"}
                         ; {:data-menu "Open menu." :class "west"}
                         ; {:data-name "Change your name." :class "west"}
                         ; {:data-team "Open team menu." :class "east"}
                         {:data-radial "Right-click." :class "north fast"}
                         )
            ; (when (not right-click-learned?) "Open chat.")
            ]])))))
